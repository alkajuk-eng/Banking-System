// Add a new account to the account select elements
function addNewSelection(accountArray,account,fullName,dob,accountNumber,sortCode) {
  ...
  // Add the account holder if the name is not already present
  ...
  // Check if this date of birth already exists
  const dobArray = accountArray.filter(
    item => item.birthDay === account.birthDay
  );

  // Add the date of birth if it is not already present
...
// Export the addNewSelection function
